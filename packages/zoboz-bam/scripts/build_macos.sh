echo "Building for MacOS" &&
  ...
  ssh dariush@$VM_MAC 'cd ~/zoboz-bam && cargo test && rustup target add aarch64-apple-darwin && cargo build --release --target aarch64-apple-darwin && rustup target add x86_64-apple-darwin && cargo build --release --target x86_64-apple-darwin' &&
